  ...
  for (RulesetConfig &x : ruleset_list) {
    ...
    if (pos != std::string::npos) {
      ...
      std::string effective_ua = x.UserAgent.empty()
          ...
          : x.UserAgent;
      // Proxy 优先级（含存在性检查）: 每条规则 ,proxy= (配置中存在) > &rules-proxy= (配置中存在) > 无
      ...
      if (!x.Proxy.empty()) {
        if (proxyExistsInConfig(x.Proxy, global.customProxyGroups)) {
          ...
          writeLog(0,
                   "  -> Per-rule proxy '" + x.Proxy + "' not found in config, falling back to &rules-proxy for ruleset '" +
                       rule_url + "': '" + rules_proxy + "'",
                   LOG_LEVEL_INFO);
        } else {
          writeLog(0,
                   "  -> Per-rule proxy '" + x.Proxy + "' not found in config, no valid fallback for ruleset '" +
                       rule_url + "'.",
                   LOG_LEVEL_WARNING);
        }
        ...
        if (proxyExistsInConfig(rules_proxy, global.customProxyGroups)) {
          ...
          writeLog(0,
                   "  -> &rules-proxy '" + rules_proxy + "' not found in config for ruleset '" +
                       rule_url + "'.",
                   LOG_LEVEL_WARNING);
        }
      }
